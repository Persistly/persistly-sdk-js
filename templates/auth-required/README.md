...
- `usage.ts` shows a Google sign-in flow. Use `signInWithProvider` for OIDC/JWT providers such as Clerk, Auth0, Supabase, Firebase, or a custom backend.

## Provider Tokens

Send provider tokens only to `signInWithGoogleIdToken` or `signInWithProvider`. Normal save, load, and sync calls use the Persistly account session returned by the SDK.
