...
- `signInWithFirebaseToken(token, { deviceLabel })`, `signInWithSupabaseToken(token, { deviceLabel })`, `signInWithAuth0Token(token, { deviceLabel })`, `signInWithProvider({ provider: "firebase" | "supabase" | "auth0", token, deviceLabel })`, `linkProvider({ provider: "firebase" | "supabase" | "auth0", token, deviceLabel })`, `listLinkedProviders()`, and `signOut()` manage Auth Bridge sessions. `accountMode: "authRequired"` keeps local saves available but prevents anonymous cloud account creation before sign-in.
